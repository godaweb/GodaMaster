...
namespace db_test
{
    public partial class EOSドラスタ受注取込Form : Form
    {
        ...
        public EOSドラスタ受注取込Form()
        {
            InitializeComponent();
        }

        private void EOSドラスタ受注取込Form_Load(object sender, EventArgs e)
        {
            this.gcMultiRow1.Template = new EOSドラスタ受注取込Template();
            ...
            iniFile = Properties.Settings.Default.EOSドラスタ受注取込ファイル;
            ...
            if (gcMultiRow.CurrentCell is ButtonCell)
            {
                switch (e.CellName)
                {
                    case "取込buttonCell":
                        ...
                        string csvFile = gcMultiRow1.ColumnHeaders[0].Cells["受注ファイルtextBoxCell"].Value.ToString();
                        ...
                        else
                        {
                            ...
                            DataTable dataTable = this.wTドラスタ受注ファイル1TableAdapter1.GetDataドラスタ受注エラーリスト();

                            プレビューForm プレビューform = new プレビューForm();
                            プレビューform.dataTable = dataTable;
                            プレビューform.rptName = "ドラスタ受注エラーリストCrystalReport";
                            プレビューform.Show();
...
//            SqlBulkCopy bulkCopy;
            ...
            //string connectionString = System.Configuration.ConfigurationSettings.AppSettings["db_test.Properties.Settings.SPEEDDBConnectionString"];
            string connectionString = db_test.Properties.Settings.Default.SPEEDDBConnectionString;
            ...
            //DataTable WTドラスタ受注ファイルdataTable = new DataTable(WTドラスタ受注ファイルdataSet.Tables["WTドラスタ受注ファイル"].TableName);
            DataRow newRow;
            ...
            using (StreamReader sr = new StreamReader(fileName, SJIS))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    //得意先コード(店コード+部門)
                    //Right$("000000" & CStr(Mid$(TextLine, 18, 4)), 6)
                    if (X[5] == "11")
                    {
                    ...
                    DMY = X[3].ToString();
                    H7 = null;
                    if (Utility.Z_Set(DMY) != 0) 
                    {
                        H7 = DateTime.Parse(Utility.Left(DMY.ToString(), 4) + "/" + Utility.Mid(DMY.ToString(), 5, 2) + "/" + Utility.Right(DMY.ToString(), 2)).AddDays(0);
                    }
                    //店納品日
                    DMY = X[3].ToString();
                    H8 = null;
                    if (Utility.Z_Set(DMY) != 0) 
                    {
                        H8 = DateTime.Parse(Utility.Left(DMY.ToString(), 4) + "/" + Utility.Mid(DMY.ToString(), 5, 2) + "/" + Utility.Right(DMY.ToString(), 2)).AddDays(1);
                    }
                    ...
                    H23 = Utility.Z_Set(1);
                    H24 = Utility.Z_Set(null);    //Z_Set(Mid$(TextLine, 104, 1)) //返品区分   //2012/1 h.yamamoto chg
                    Err_FlgB = null;
                    Err_FlgC = null;
                    //ボディー -------------------------------------------------------
                    ...
                    B4 = Utility.Left(Utility.S_Set(X[14]),13);    //S_Set(Mid$(TextLine, 6, 13)) //商品コード  //2012/1 h.yamamoto chg
                    ...
                    NSYO_TEKI = "";   MAKER_HIN = ""; TANA_BAN = "";  MAKER_NM = "";
                    ...
                    newRow = WTドラスタ受注ファイルdataTable.NewRow();
                    ...
                    newRow["新消費税適用"] = NSYO_TEKI;  //日付
                    //newRow["新消費税適用"] = DBNull.Value;
                    newRow["明細摘要"] = null;
                    newRow["発注番号"] = 0;
                    newRow["発注連番"] = 0;
                    ...
                    newRow["仕入先更新フラグ"] = 0;
                    newRow["オペレーターコード"] = "92";
                    newRow["修正オペレーターコード"] = null;
                    newRow["受注行"] = B3;
                    newRow["処理月日"] = DBNull.Value;
                    newRow["管理年月"] = 0;
                    newRow["受注行番号"] = B3;
                    newRow["定価"] = Utility.Z_Set(table.Rows[0]["定価"]);
                    newRow["納品掛率"] = RITU;
                    newRow["原価掛率"] = GK_RITU;
                    newRow["発注有無区分"] = 0;
                    newRow["回答納期"] = DBNull.Value;
                    newRow["在庫数"] = ZAIKO;
                    newRow["受注残数"] = JUCHUZAN;
                    newRow["発注残数"] = HACHUZAN;
                    newRow["出荷指示発行フラグ"] = 0;
                    newRow["商品注意事項"] = null;
                    newRow["発注摘要"] = null;
                    newRow["売上区分コード"] = 1;
                    ...
                    newRow["単価更新フラグ"] = 0;
                    newRow["登録番号"] = Utility.Z_Set(X[0]);
                    newRow["ドラスタ発注番号"] = Utility.Z_Set(X[1]);
                    newRow["客注番号"] = X[9];
                    newRow["発注数"] = 0;

                    WTドラスタ受注ファイルdataTable.Rows.Add(newRow);
                    ...
                    {
                        //DMY = DMY + Err_FlgB;
                        DMY = DMY + Err_FlgB.ToString();
                    }
                    if (Err_FlgC != null)
                    {
                        //DMY = DMY + Err_FlgC;
                        DMY = DMY + Err_FlgC.ToString();
                    }
                    if (DMY == null)
                    {
                        strDMY = null;
                    ...
                    this.wTドラスタ受注ファイルTableAdapter1.Insert(
                         1,
                         3,
                         1,
                         0,
                         ...
                         Utility.Right(B12.ToString(),2),  //4
                         ...
                         Convert.ToInt32(H23),
                         Convert.ToInt32(H24),
                         Convert.ToDecimal(B19),
                         Convert.ToDecimal(B20),
                         Convert.ToDecimal(B21),
                         Convert.ToDecimal(B22),
                         Utility.S_Set(X[13]),
                         Utility.S_Set(X[23]),
                         null,
                         Convert.ToBoolean(0),
