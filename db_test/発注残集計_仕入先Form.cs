...
namespace db_test
{
    public partial class 発注残集計_仕入先Form : Form
    {
        private string receiveDataSirsaki = "";

        public 発注残集計_仕入先Form()
        {
            InitializeComponent();
        }

        private void 発注残集計_仕入先Form_Load(object sender, EventArgs e)
        {
            this.gcMultiRow1.Template = new 発注残集計_仕入先Template();
                ...
                case Keys.Enter:
                    ...
                    foreach (Cell cell in this.gcMultiRow1.SelectedCells)
                    {
                        if (cell.RowIndex >= 0)
                        {
                            ...
                            if (Val != null)
                            {
                                発注残集計_仕入先内訳Form fm = new 発注残集計_仕入先内訳Form();
                                fm.Owner = this;
        ...
        private int createData()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(0, "仕入先コード");
                }
            }
            return 0;
        }
