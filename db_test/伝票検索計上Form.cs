...
namespace db_test
{
    public partial class 伝票検索計上Form : Form
    {
        ...
        private void 伝票検索計上Form_Load(object sender, EventArgs e)
        {
            this.gcMultiRow1.Template = new 伝票検索計上_ヘッダTemplate();
            this.gcMultiRow1.MultiSelect = false;
            this.gcMultiRow1.AllowUserToAddRows = false;
            this.gcMultiRow1.ScrollBars = ScrollBars.None;

            this.gcMultiRow2.Template = new 伝票検索計上左Template();
            ...
            this.gcMultiRow3.Template = new 伝票検索計上右Template();
            ...
            this.gcMultiRow1.ShortcutKeyManager.Register(new 伝票検索計上FunctionKeyAction(Keys.F3), Keys.F3);
            this.gcMultiRow1.ShortcutKeyManager.Register(new 伝票検索計上FunctionKeyAction(Keys.F5), Keys.F5);
            this.gcMultiRow1.ShortcutKeyManager.Register(new 伝票検索計上FunctionKeyAction(Keys.F9), Keys.F9);
            this.gcMultiRow1.ShortcutKeyManager.Register(new 伝票検索計上FunctionKeyAction(Keys.F10), Keys.F10);
            ...
            switch (Owner.Text)
            {
                case "売上計上入力Form":
                    ...
                    gcMultiRow2.ColumnHeaders[0].Cells["columnHeaderCell36"].Value = "売上日";
                    gcMultiRow2.ColumnHeaders[0].Cells["columnHeaderCell32"].Value = "受注番号";
                ...
                case "仕入計上入力Form":
                    ...
                    gcMultiRow2.ColumnHeaders[0].Cells["columnHeaderCell36"].Value = "仕入日";
                    gcMultiRow2.ColumnHeaders[0].Cells["columnHeaderCell32"].Value = "発注番号";
            ...
            gcMultiRow1.ColumnHeaders[0].Cells["日付textBoxCell"].Value = DateTime.Today.Year + "/" + DateTime.Today.Month + "/" + DateTime.Today.Day;
        ...
        void gcMultiRow1_CellEndEdit(object sender, CellEndEditEventArgs e)
        {
            switch (e.CellName)
            {
                case "伝票番号textBoxCell":
                    ...
                    if (ret > 0)
                    {
                        MessageBox.Show("データがありません。", "伝票検索");
        ...
        void gcMultiRow1_CellValidated(object sender, CellEventArgs e)
        {
            switch (e.CellName)
            {
                case "事業所コードtextBoxCell":
                    ...
                    {
                        if (Utility.existCombo(comboBoxCell01, (string)this.gcMultiRow1.ColumnHeaders[0].Cells["事業所コードtextBoxCell"].Value))
                        {
                            this.gcMultiRow1.ColumnHeaders[0].Cells["事業所gcComboBoxCell"].Value = this.gcMultiRow1.ColumnHeaders[0].Cells["事業所コードtextBoxCell"].Value;
                        }
                    }
                    break;
                case ("事業所gcComboBoxCell"):
                    if ((string)this.gcMultiRow1.ColumnHeaders[0].Cells["事業所gcComboBoxCell"].Value != null)
                    {
        ...
        void gcMultiRow1_CellEditedFormattedValueChanged(object sender, CellEditedFormattedValueChangedEventArgs e)
        {
            switch (e.CellName)
            {
                case ("事業所gcComboBoxCell"):
                    ...
                    {
                        this.gcMultiRow1.ColumnHeaders[0].Cells["事業所textBoxCell"].Value = Utility.GetCode("T事業所マスタ", "事業所コード", "事業所名", (string)this.gcMultiRow1.ColumnHeaders[0].Cells["事業所gcComboBoxCell"].EditedFormattedValue);
                    }
                    break;
                ...
                switch (Owner.Text)
                {
                    case "売上計上入力Form":
                        ...
                        if (売上計上入力form != null)
                        {
                            ...
                            this.Hide();
                            //売上計上入力form.ReceiveDataToksaki = gcMultiRow1.GetValue(e.RowIndex, e.CellIndex).ToString();
                            売上計上入力form.ReceiveDataUriageDenpyo = Val.ToString().Trim();
                        }
                        break;
                    case "仕入計上入力Form":
                        ...
                        if (仕入計上入力form != null)
                        {
                            ...
                            仕入計上入力form.ReceiveDataSirDenpyo = Val.ToString().Trim();
                        }
                        break;
                }
            }
                    ...
                    if (cell.RowIndex >= 0)
                    {
                        ...
                        if (Val != null)
                        {
                            ...
                            else if (Owner.Text == "仕入計上入力Form")
                            {
                                ...
                                if (仕入計上入力form != null)
                                {
                                    //仕入計上入力form.ReceiveDataToksaki = gcMultiRow1.GetValue(e.RowIndex, e.CellIndex).ToString();
                                    仕入計上入力form.ReceiveDataSirDenpyo = Val.ToString().Trim();
                                }
                            }
        ...
        void gcMultiRow2_RowEnter(object sender, CellEventArgs e)
        {
            ...
            {
                string num = gcMultiRow2.GetValue(e.RowIndex, "伝票番号textBoxCell").ToString();
        ...
        private int execProcedure_Head()
        {
            ...
            伝票ヘッダTable = this.伝票検索計上ヘッダTableAdapter1.GetData伝票検索計上ヘッダ(parm処理コード, parm入力区分, parm事業所コード, parm担当者コード, parm取引先コード, parm日付, parm管理年月, parm伝票番号, parmフリガナ);
        ...
        private int execProcedure_Meisai(string 伝票番号)
        {
            ...
            伝票明細Table = this.伝票検索計上明細TableAdapter1.GetData(parm処理コード, 伝票番号);
                    ...
                    if (Val != null)
                    {
                        switch (Owner.Text)
                        {
                            case "売上計上入力Form":
                                ...
                                if (売上計上入力form != null)
                                {
                                    売上計上入力form.ReceiveDataUriageDenpyo = gcMultiRow2.CurrentRow.Cells[0].Value.ToString();
                            ...
                            case "仕入計上入力Form":
                                ...
                                if (仕入計上入力form != null)
                                {
                                    仕入計上入力form.ReceiveDataSirDenpyo = gcMultiRow2.CurrentRow.Cells[0].Value.ToString();
