...
namespace db_test
{
    public partial class 発注残集計_仕入先内訳Form : Form
    {
        ...
        public 発注残集計_仕入先内訳Form()
        {
            InitializeComponent();
        }

        private void 発注残集計_仕入先内訳Form_Load(object sender, EventArgs e)
        {
        ...
        private int createData()
        {
            ...
            // ストアド プロシージャ名を指定
            command.CommandText = "発注残集計_仕入先別内訳";
            ...
            if ((String)gcMultiRow1.ColumnHeaders[0].Cells["仕入先コードtextBoxCell"].Value != null && (String)gcMultiRow1.ColumnHeaders[0].Cells["仕入先コードtextBoxCell"].Value != "*")
            {
                command.Parameters.AddWithValue("@仕入先コード", (String)gcMultiRow1.ColumnHeaders[0].Cells["仕入先コードtextBoxCell"].Value);
            }
            else
            {
