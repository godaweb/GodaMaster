...
namespace db_test
{
    public partial class 売上計上_ドラスタ対応_ヘッダForm : Form
    {
        private string sendドラスタ_ヘッダ_Data = null;
        private string receiveドラスタ_ヘッダ_Data = null;
        private string[] arr = null;

        public 売上計上_ドラスタ対応_ヘッダForm()
        {
            InitializeComponent();
        }

        private void 売上計上_ドラスタ対応_ヘッダForm_Load(object sender, EventArgs e)
        {
            this.gcMultiRow1.Template = new 売上計上_ドラスタ対応_ヘッダTemplate();
            ...
            this.gcMultiRow1.ColumnHeaders[0].Cells["直送区分textBoxCell"].Value = arr[2];
            this.gcMultiRow1.ColumnHeaders[0].Cells["ＥＯＳ区分textBoxCell"].Value = arr[3];
            this.gcMultiRow1.ColumnHeaders[0].Cells["客注区分textBoxCell"].Value = arr[4];
            gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "経費区分textBoxCell");
        ...
        void gcMultiRow1_NewCellPositionNeeded(object sender, NewCellPositionNeededEventArgs e)
        {
            switch (e.CellName)
            {
                case "経費区分textBoxCell":
                    if (e.MoveStatus == MoveStatus.MoveToNextCell)
                    {
                        e.NewCellPosition = new CellPosition(CellScope.ColumnHeader, 0, "客注区分textBoxCell");
                    }
                    break;
            }
        }
        ...
        void gcMultiRow1_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            switch (gcMultiRow1.CurrentCellPosition.CellName)
            {
                case "戻るbuttonCell":
                    ...
                    {
                        case Keys.Enter:
                            if (this.gcMultiRow1.ColumnHeaders[0].Cells["社コードtextBoxCell"].Value == null)
                            {
                                gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "社コードtextBoxCell");
                                return;
                            }
                            if (this.gcMultiRow1.ColumnHeaders[0].Cells["経費区分textBoxCell"].Value == null)
                            {
                                gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "経費区分textBoxCell");
                                return;
                            }
                            if (this.gcMultiRow1.ColumnHeaders[0].Cells["直送区分textBoxCell"].Value == null)
                            {
                                gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "直送区分textBoxCell");
                                return;
                            }
                            if (this.gcMultiRow1.ColumnHeaders[0].Cells["ＥＯＳ区分textBoxCell"].Value == null)
                            {
                                gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "ＥＯＳ区分textBoxCell");
                                return;
                            }
                            if (this.gcMultiRow1.ColumnHeaders[0].Cells["客注区分textBoxCell"].Value == null)
                            {
                                gcMultiRow1.CurrentCellPosition = new GrapeCity.Win.MultiRow.CellPosition(GrapeCity.Win.MultiRow.CellScope.ColumnHeader, 0, "客注区分textBoxCell");
                                return;
                            }
                            ...
                            {
                                string buf = null;
                                buf = this.gcMultiRow1.ColumnHeaders[0].Cells["社コードtextBoxCell"].Value.ToString();
                                buf += "," + this.gcMultiRow1.ColumnHeaders[0].Cells["経費区分textBoxCell"].Value.ToString();
                                buf += "," + this.gcMultiRow1.ColumnHeaders[0].Cells["直送区分textBoxCell"].Value.ToString();
                                buf += "," + this.gcMultiRow1.ColumnHeaders[0].Cells["ＥＯＳ区分textBoxCell"].Value.ToString();
                                buf += "," + this.gcMultiRow1.ColumnHeaders[0].Cells["客注区分textBoxCell"].Value.ToString();
                                fm.Receiveドラスタ_ヘッダ_Data = buf;
        ...
        void gcMultiRow1_CellValidating(object sender, CellValidatingEventArgs e)
        {
            switch (e.CellName)
            {
                ...
                case "直送区分textBoxCell":
                    ...
                    else if ((string)e.FormattedValue == "3")
                    {
                        e.Cancel = false;
                    }
                    else
                    ...
                    break;
                case "ＥＯＳ区分textBoxCell":
                    if (e.FormattedValue == null)
                    {
                        e.Cancel = true;
                    }
                    else if ((string)e.FormattedValue == "11")
                    {
                        e.Cancel = false;
                    }
                    else if ((string)e.FormattedValue == "21")
                    {
                        e.Cancel = false;
                    }
                    else
