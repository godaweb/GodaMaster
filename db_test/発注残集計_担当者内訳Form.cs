...
namespace db_test
{
    public partial class 発注残集計_担当者内訳Form : Form
    {
        ...
        public 発注残集計_担当者内訳Form()
        {
            InitializeComponent();
        }

        private void 発注残集計_担当者内訳Form_Load(object sender, EventArgs e)
        {
        ...
        private int createData()
        {
            ...
            // ストアド プロシージャ名を指定
            command.CommandText = "発注残集計_担当者別内訳";
            ...
            {
                command.Parameters.AddWithValue("@仕入担当者コード", (String)gcMultiRow1.ColumnHeaders[0].Cells["担当者コードtextBoxCell"].Value);
            }
            else
            {
                command.Parameters.AddWithValue("@仕入担当者コード", DBNull.Value);
            }
