...
namespace db_test
{
    public partial class 受注残集計_得意先内訳Form : Form
    {

        private string receiveDataToksaki = "";

        public 受注残集計_得意先内訳Form()
        {
            InitializeComponent();
        }
        ...
        public string ReceiveDataToksaki
        {
            set
            {
                ...
                this.gcMultiRow1.ColumnHeaders[0].Cells["得意先コードtextBoxCell"].Value = receiveDataToksaki.ToString();
                SendKeys.Send("{ENTER}");
            }
            get
            {
        ...
        private void 受注残集計_得意先内訳Form_Load(object sender, EventArgs e)
        {
        ...
        private int createData()
        {
            ...
            // ストアド プロシージャ名を指定
            command.CommandText = "受注残集計_得意先別内訳";
            ...
            if ((String)gcMultiRow1.ColumnHeaders[0].Cells["得意先コードtextBoxCell"].Value != null && (String)gcMultiRow1.ColumnHeaders[0].Cells["得意先コードtextBoxCell"].Value != "*")
            {
                command.Parameters.AddWithValue("@得意先コード", (String)gcMultiRow1.ColumnHeaders[0].Cells["得意先コードtextBoxCell"].Value);
            }
            else
            {
